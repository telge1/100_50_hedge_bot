...
mapfile -t _account_data < <(
  python3 - "${CONFIG_FILE}" "${BOT_NAME}" "${SIDE}" <<'PY'
import sys
...
account_name = None
if side == "long":
    account_name = profile.get("long_account")
else:
    account_name = profile.get("short_account")

fallback_map = {
    "long_bot_1": ["master", "Long_bot_1"],
    "long_bot_2": ["Long_bot_2", "bot_2"],
    "short_bot_1": ["sub", "Short_bot_1"],
    "short_bot_2": ["Short_bot_2", "bot_2_sub"],
}
if not account_name:
...
if [[ -z "${API_KEY}" || -z "${SECRET_KEY}" ]]; then
  echo "ERROR: credentials missing for account ${ACCOUNT_NAME}" >&2
  return 1
fi
