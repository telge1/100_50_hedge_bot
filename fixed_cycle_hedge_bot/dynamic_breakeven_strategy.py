...
class DynamicBreakevenHedgeStrategy(HedgeStrategy):
    ...
    def on_fill(
        ...
        if fill_event.purpose == self.config.long_reduce_purpose:
            ...
            edge = short_entry * self.config.edge_buffer_pct
            short_exit_price = max(self.config.min_short_price, short_entry - needed_short_move - edge)
            state["awaiting_short_fill"] = True
            state["last_short_exit_price"] = short_exit_price
            ...
            traces = [
                ...
                CalculationTrace(
                    ...
                    result={"needed_short_move": needed_short_move},
                ),
                CalculationTrace(
                    name="short_exit_price",
                    formula="short_exit_price = short_entry - needed_short_move - edge",
                    inputs={
                        "short_entry": short_entry,
                        "needed_short_move": needed_short_move,
                        "edge": edge,
                    },
