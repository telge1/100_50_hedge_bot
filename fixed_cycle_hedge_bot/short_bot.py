...
class FixedCycleHedgeStrategy(HedgeStrategy):
    ...
    def on_start(
        ...
        state.setdefault("cycle_waiting_for_long_tp", False)
        state.setdefault("pending_short_cycle_index", 0)
        state.setdefault("long_tp_pending_cycle", 0)
        state.setdefault("short_reduce_pending", False)
        state.setdefault("cycle_completed_count", 0)
        state.setdefault("cycle_short_reduce_filled", False)
        state.setdefault("cycle_long_tp_filled", False)
        ...
        *,
        ...
        logger.debug(
            "fixed_cycle_rebuild_entry %s",
            {
                ...
                "cycle_waiting_for_long_tp": bool(state.get("cycle_waiting_for_long_tp")),
    ...
    def _build_downside_cycle_intents(
        ...
        if not state.get("short_add_rebuild_allowed", True):
            context.audit.log_event(
                "fixed_cycle_downside_skip",
                strategy=self.name,
                skip_reason="short_add_locked",
            )
            return intents
        ...
        short_reduce_pending = bool(state.get("short_reduce_pending"))
        long_tp_pending_cycle = int(state.get("long_tp_pending_cycle") or 0)
        waiting_for_long_tp = bool(state.get("cycle_waiting_for_long_tp"))
        ...
        logger.debug(
            "fixed_cycle_downside_build_inputs %s",
            {
                ...
                "long_tp_pending_cycle": long_tp_pending_cycle,
                "cycle_waiting_for_long_tp": waiting_for_long_tp,
            },
        )
        ...
        if short_cycle_number <= self.config.max_cycles:
            ...
            if waiting_for_long_tp:
                skip_signature = {
                    "reason": "waiting_for_long_tp",
                ...
                skip_event_kwargs = {
                    "strategy": self.name,
                    "skip_reason": "waiting_for_long_tp",
                    "cycle_number": short_cycle_number,
                    "purpose": purpose,
                    "long_tp_pending_cycle": long_tp_pending_cycle,
                }
            ...
            if skip_signature and skip_event_kwargs:
                ...
                raw_trigger_price = long_reference * (1 + short_distance_pct)
                trigger_price = self._normalize_price(raw_trigger_price, runtime_state)
                # trigger stays above reference for short
                ...
                logger.debug(
                    "fixed_cycle_downside_cycle_evaluated %s",
                    {
                        ...
                        "step_index": 1,
                        "side": "short",
                        "short_fill_price": short_fill_price,
                        "cycle_entry_price": cycle_entry_price,
                        "live_reference_price": reference_price,
                        "short_reference_candidate": short_reference_candidate,
                        "final_short_reference": short_reference,
                ...
                if not will_append_intent:
                    ...
                    context.audit.log_event(
                        "fixed_cycle_short_reduce_planned",
                        strategy=self.name,
                        ...
                        live_reference_price=reference_price,
                        final_short_reference=short_reference,
                        trigger_formula="final_short_reference * (1 + distance_pct)",
                ...
                if will_append_intent:
                    state["short_add_rebuild_allowed"] = False
        long_intents = [intent for intent in intents if intent.side == "long"]
        short_intents = [intent for intent in intents if intent.side == "short"]
        first_long_purpose = self._cycle_purpose("long", 1)
        first_short_purpose = self._cycle_purpose("short", 1)
    ...
    def _build_short_tp_follow_up(
        ...
        state = runtime_state.strategy_state
        cycle_state = self._ensure_cycle_state(runtime_state)
        cycle_index = int(state.get("long_tp_pending_cycle") or 0)
        if cycle_index <= 0 or not state.get("cycle_waiting_for_long_tp"):
            return []
        ...
        if not bool(long_fill.get("closed_pnl_ready")):
            ...
            if not closed_pnl_ready:
                ...
                state["cycle_waiting_for_long_tp"] = True
                cycle_state["cycle_waiting_for_long_tp"] = True
                state["long_tp_pending_cycle"] = cycle_index
                cycle_state["long_tp_pending_cycle"] = cycle_index
                return []
    ...
    def _build_exit_intents(
        ...
        if state.get("short_reduce_pending"):
            context.audit.log_event(
                "fixed_cycle_exit_skip",
                strategy=self.name,
                skip_reason="short_reduce_pending",
                short_reduce_pending=True,
            )
            return intents
    ...
    def _advance_cycle_from_fill(
        ...
        if purpose in {"REFILL_LONG", "REFILL_SHORT"}:
            ...
            if refill_state.get("REFILL_LONG") and refill_state.get("REFILL_SHORT"):
                ...
                state["cycle_short_reduce_filled"] = False
                state["cycle_long_tp_filled"] = False
                state["cycle_waiting_for_long_tp"] = False
                state["pending_short_cycle_index"] = 0
                state["long_tp_pending_cycle"] = 0
                state["short_reduce_pending"] = False
                state["exit_rebuild_allowed"] = True
                state["short_add_rebuild_allowed"] = True
                state["refill_state"] = {}

                cycle_state["short_reduce_pending"] = False
                cycle_state["cycle_waiting_for_long_tp"] = False
                cycle_state["pending_short_cycle_index"] = 0
                cycle_state["long_tp_pending_cycle"] = 0
        ...
        if "_SHORT_" in fill_event.purpose and "SHORT_REDUCE" in fill_event.purpose:
            state["short_reduce_pending"] = fill_event.status != "FILLED"
            cycle_state["short_reduce_pending"] = state["short_reduce_pending"]
            state["cycle_short_reduce_filled"] = True
            state["short_add_rebuild_allowed"] = True
            logger.info(
                "[CYCLE-SHORT-REDUCE] purpose=%s status=%s long_flag=%s cycle_count=%s",
                fill_event.purpose,
                fill_event.status,
                state.get("cycle_short_reduce_filled"),
                state.get("cycle_completed_count"),
            )
            if order_fully_completed:
                state["exit_rebuild_allowed"] = True
        if "_SHORT_" in fill_event.purpose and order_fully_completed:
        ...
        if "_LONG_" in fill_event.purpose and (
            "LONG_TP" in fill_event.purpose or "LONG_REDUCE" in fill_event.purpose
        ):
            state["cycle_long_tp_filled"] = True
        ...
        if "_LONG_" in fill_event.purpose:
            if "LONG_TP" in fill_event.purpose or "LONG_REDUCE" in fill_event.purpose:
                state["cycle_long_tp_filled"] = True
                logger.info(
                    "[CYCLE-SHORT] purpose=%s status=%s short_flag=%s cycle_count=%s",
                    fill_event.purpose,
                    fill_event.status,
                    state.get("cycle_long_tp_filled"),
                    state.get("cycle_completed_count"),
                )
            ...
            if order_fully_completed and "SHORT_REDUCE" in fill_event.purpose:
                state["exit_rebuild_allowed"] = True
            logger.info(
                "[CYCLE-CHECK] short_flag=%s long_flag=%s",
                state.get("cycle_short_add_filled"),
                state.get("cycle_long_tp_filled"),
            )
            if state.get("cycle_short_reduce_filled") and state.get("cycle_long_tp_filled"):
            ...
            state["short_add_rebuild_allowed"] = True
            logger.info(
                "[CYCLE-COMPLETE] reset triggered",
                extra={"cycle_completed_count": state.get("cycle_completed_count")},
            )
    ...
    def _default_cycle_state(self) -> dict:
        return {
            ...
            "cycle_waiting_for_long_tp": False,
            "pending_short_cycle_index": 0,
            "long_tp_pending_cycle": 0,
            "pending_short_cycle_index": 0,
    ...
    def _ensure_cycle_state(self, runtime_state: RuntimeState) -> dict:
        ...
        state["cycle_waiting_for_long_tp"] = bool(state.get("cycle_waiting_for_long_tp")) or bool(
            cycle_state.get("cycle_waiting_for_long_tp")
        )
        state["pending_short_cycle_index"] = max(
            int(state.get("pending_short_cycle_index") or 0),
            int(cycle_state.get("pending_short_cycle_index") or 0),
        )
        state["long_tp_pending_cycle"] = max(
