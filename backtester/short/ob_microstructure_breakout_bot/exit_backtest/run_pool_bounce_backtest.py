"""Run MD-rule 5m pool bounce backtest (gap filter + OB/delta columns)."""
...
from ob_microstructure_breakout_bot.exit_backtest.pool_bounce_backtest import (
    ...
    WATCH_BEFORE_PCT,
    analyze_signal_bounce_backtest,
    summarize_backtest,
)
...
def _md_report(
    *,
    ...
    lines = [
        f"# 5m Pool Bounce Backtest ({symbol})",
        "",
        "Implements `results/ob_pool_5m_bounce_rule.md`.",
        "",
        f"- universe: `{universe}`",
        f"- n_signals: **{n_signals}**",
        f"- min TP room (pool bottom → next lower top): **{MIN_LOWER_GAP_PCT}%**",
        f"- short SL: pool top + **{SL_ABOVE_POOL_TOP_PCT}%**",
        ...
        + "**",
        "",
        f"## Tradeable gap filter (TP room >= {MIN_LOWER_GAP_PCT}%)",
        "",
        ...
        f"- mean lower gap %: **"
        + (
            f"{tradeable['mean_lower_gap_pct']:.2f}%"
            if tradeable.get("mean_lower_gap_pct") is not None
            else "—"
        )
        + "**",
        "",
    ...
    lines.extend(
        [
            ...
            "`next_lower_pool_gap_pct`, `tradeable_gap`, `watch_ts`, "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="5m pool bounce backtest (MD rule)")
    parser.add_argument("--symbol", default="DOGEUSDT")
    parser.add_argument(
        "--universe",
        choices=("locked10", "scanner", "full"),
        default="locked10",
        help="locked10 / scanner_breakout-only / full=scanner∪strong∪fakeout (OB window)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Only trade top-N upper clusters by strength (default 2 = Rank1+Rank2)",
    )
    ...
    parser.add_argument(
        "--out-md",
        type=Path,
        default=_REPO / "results" / "ob_pool_5m_bounce_backtest.md",
    )
    ...
    if args.universe == "scanner":
        ...
        # All Phase-A longs inside the OB+trades coverage window.
        longs = load_full_history_longs(
            symbol=symbol,
            scanner_path=args.signals,
            strong_path=args.strong,
            fakeout_path=args.fakeouts,
            sources=["scanner_breakout", "strong_breakout", "fakeout"],
        )
    else:
    ...
    for row in longs:
        ...
        try:
            ...
            res = analyze_signal_bounce_backtest(
                symbol,
        ...
        for r in (res.get("rows") or [])[:2]:
            print(
                f"  rank{r.get('rank')} gap={r.get('next_lower_pool_gap_pct')} "
    ...
    payload = {
        ...
        "params": {
            ...
            "sl_above_pool_top_pct": SL_ABOVE_POOL_TOP_PCT,
