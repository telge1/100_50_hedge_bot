...
_confirmed_pnl_path_logic_path = project_root / "fixed_cycle_hedge_bot" / "confirmed_pnl_path_logic.py"
_confirmed_pnl_path_logic_spec = importlib.util.spec_from_file_location(
    "_dashboard_confirmed_pnl_path_logic",
    _confirmed_pnl_path_logic_path,
)
if _confirmed_pnl_path_logic_spec is None or _confirmed_pnl_path_logic_spec.loader is None:
    raise ImportError(f"unable to load confirmed pnl path logic from {_confirmed_pnl_path_logic_path}")
_confirmed_pnl_path_logic = importlib.util.module_from_spec(_confirmed_pnl_path_logic_spec)
_confirmed_pnl_path_logic_spec.loader.exec_module(_confirmed_pnl_path_logic)
_shared_confirmed_row_dedupe_key = _confirmed_pnl_path_logic.confirmed_row_dedupe_key
validate_confirmed_pnl_row_for_path = _confirmed_pnl_path_logic.validate_confirmed_pnl_row_for_path
_finalize_confirmed_rows = _confirmed_pnl_path_logic.finalize_confirmed_rows
_build_confirmed_pnl_index_from_rows = _confirmed_pnl_path_logic.build_confirmed_pnl_index_from_rows
_filter_confirmed_rows_for_trade_block = _confirmed_pnl_path_logic.filter_confirmed_rows_for_trade_block
...
def _compute_trade_end_profit_from_existing_details(
    record: dict[str, Any],
) -> tuple[float | None, int, list[str]]:
    """
    Berechnet einen konsistenten Endprofit aus bestehenden Detailzeilen.

    Nutzt ausschließlich bereits vorhandene Details (Breakdown/Confirmed-Details),
    ohne neue Quellen zu laden.
    """
    ...
    for detail in details:
        raw = (
            detail.get("pnl")
            if detail.get("pnl") is not None
            else detail.get("pnl_usdt")
            if detail.get("pnl_usdt") is not None
            else detail.get("realized_pnl")
        )
        value = _safe_wallet_float(raw)
        if value is None:
            continue
        pnl_values.append(value)
        purpose_text = str(detail.get("purpose") or "").strip()
        if purpose_text:
            purposes.add(purpose_text)

    if not pnl_values:
        return None, 0, sorted(purposes)

    end_profit = float(sum(pnl_values))
...
def _build_profit_trade_summary(
    ...
    return {
        ...
        "profit_usdt": record.get("profit_usdt")
        if record.get("profit_usdt") is not None
...
def _build_confirmed_detail_rows(record: dict[str, Any], confirmed_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for entry in confirmed_rows:
        ...
        bot_side_value: str | None = None
        if bot_name_value:
            normalized_bot = str(bot_name_value).strip().lower()
            if normalized_bot.startswith("short_bot_"):
                bot_side_value = "short"
            elif normalized_bot.startswith("long_bot_"):
                bot_side_value = "long"
...
def _collect_closed_history_paths_for_profile(profile: str, bot_side: str | None = None) -> list[Path]:
    normalized = _normalize_dashboard_profile(profile, fallback_to_main=False)
    if not normalized or normalized == "main":
        return [DASHBOARD_CLOSED_PNL_HISTORY_FILE]
...
def _collect_history_paths_for_profile(profile: str, bot_side: str | None = None) -> list[Path]:
    paths = _collect_closed_history_paths_for_profile(profile, bot_side)
    paths.extend(_collect_confirmed_history_paths(profile, bot_side))
    return paths
...
def _collect_confirmed_trade_start_times(
    ...
    confirmed_rows: list[dict[str, Any]] | None = None,
) -> dict[str, datetime]:
...
def _filter_confirmed_rows(rows: list[dict[str, Any]], trade_block_id: str) -> list[dict[str, Any]]:
    return _filter_confirmed_rows_for_trade_block(rows, trade_block_id)
...
def _build_confirmed_pnl_index(
    ...
    """Load confirmed PnL history once and group rows by trade_block_id."""
    paths = _collect_confirmed_pnl_paths_for_view(profile, bot_side)
    all_rows = _collect_confirmed_order_pnl_rows_from_paths(paths)
    return _build_confirmed_pnl_index_from_rows(all_rows)


def _flatten_confirmed_pnl_index(
    confirmed_index: dict[str, list[dict[str, Any]]],
) -> list[dict[str, Any]]:
    flattened: list[dict[str, Any]] = []
    for tbid in sorted(confirmed_index):
        flattened.extend(confirmed_index[tbid])
    return flattened
...
def _history_trade_events_from_entries(
    entries: list[dict[str, Any]],
    profile: str,
    bot_side: str | None = None,
) -> list[dict[str, Any]]:
    ...
    for entry in entries:
        ...
        if record.get("closed_pnl") is not None and not record.get("__event") and not record.get("event"):
            record["__event"] = "confirmed_order_pnl_history"
        else:
...
def _load_history_trade_events_for_profile(
    profile: str,
    bot_side: str | None = None,
    confirmed_rows: list[dict[str, Any]] | None = None,
) -> list[dict[str, Any]]:
    closed_entries = _load_history_entries_from_paths(
        _collect_closed_history_paths_for_profile(profile, bot_side)
    )
    if confirmed_rows is None:
        confirmed_entries = _collect_confirmed_order_pnl_rows_from_paths(
            _collect_confirmed_history_paths(profile, bot_side)
        )
    else:
        confirmed_entries = confirmed_rows
    return _history_trade_events_from_entries(
        closed_entries + confirmed_entries,
        profile,
        bot_side,
    )
...
def _load_trade_blocks_for_profile(
    ...
    *,
    ...
    history_events = _load_history_trade_events_for_profile(
        profile, bot_side, confirmed_rows=confirmed_rows
    )
    if history_events:
...
def _is_bot_clearly_stopped(status: dict[str, Any] | None) -> bool:
    if not status:
        return True
    status_value = str(status.get("status") or "").strip().lower()
    if status_value in {"stopped", "idle", "inactive", "closed"}:
        return True
    pid = status.get("pid")
    pid_active = pid is not None and str(pid).strip().lower() not in {"", "null", "none", "0"}
    if not pid_active and not bool(status.get("start_requested")):
        return True
    return False


def _is_bot_running(status: dict[str, Any] | None) -> bool:
    if _is_bot_clearly_stopped(status):
        return False
    if not status:
        return False
...
def _get_confirmed_pnl_rows_for_trade_block(
    ...
    bot_side: str | None = None,
    bot_name: str | None = None,
    confirmed_index: dict[str, list[dict[str, Any]]] | None = None,
) -> list[dict[str, Any]]:
    if not trade_block_id:
        return []
    _ = bot_name
    if confirmed_index is not None:
        return list(confirmed_index.get(trade_block_id, []))
    paths = _collect_confirmed_pnl_paths_for_view(profile, bot_side)
...
) -> tuple[float, int, list[str], dict[str, int], dict[str, int]]:
    pnl_sum = 0.0
    pnl_count = 0
    purposes: set[str] = set()
    purpose_counts: dict[str, int] = {}
    pnl_scope_counts: dict[str, int] = {}
    for row in confirmed_rows:
        raw_pnl = (
            row.get("closed_pnl")
        ...
        pnl_scope = str(row.get("pnl_scope") or "").strip().lower() or "unknown"
        pnl_scope_counts[pnl_scope] = pnl_scope_counts.get(pnl_scope, 0) + 1
    return round(pnl_sum, 8), pnl_count, sorted(purposes), purpose_counts, pnl_scope_counts


def _apply_confirmed_end_profit_to_record(
    record: dict[str, Any],
    confirmed_rows: list[dict[str, Any]],
) -> tuple[float, int, list[str]]:
    end_profit, pnl_count, purposes, _purpose_counts, _pnl_scope_counts = _sum_confirmed_closed_pnl_rows(
        confirmed_rows
    )
    record["total_trade_pnl"] = end_profit
    record["end_profit"] = end_profit
    record["profit_usdt"] = end_profit
    record["confirmed_pnl_row_count"] = len(confirmed_rows)
...
def _has_active_trade_state(
    ...
    if _is_bot_clearly_stopped(status):
        if has_position:
            logger.info(
                "[dashboard] profit_trades_active_state_stopped_with_position",
                {
                    "long_qty": long_qty,
                    "short_qty": short_qty,
            ...
            return True
        logger.info(
            "[dashboard] profit_trades_stale_state_ignored_bot_stopped",
            {
                "long_qty": long_qty,
                "short_qty": short_qty,
                "status": (status.get("status") if isinstance(status, dict) else None),
                "pid": (status.get("pid") if isinstance(status, dict) else None),
                "start_requested": (status.get("start_requested") if isinstance(status, dict) else None),
            },
        )
        return False
...
def _collect_process_rows_for_bot(
    ...
    if not _has_active_trade_state(state, strategy_state, status):
        return None, None
    runner_active = _is_bot_running(status)
    stopped_with_position = _is_bot_clearly_stopped(status) and not runner_active
    trade_block_id = str(
        state.get("trade_block_id") or strategy_state.get("trade_block_id") or strategy_state.get("last_trade_block_id") or ""
    ).strip()
    if not trade_block_id:
        return None, f"trade_block_id missing for active bot {bot_name}"
    if stopped_with_position:
        logger.warning(
            "[dashboard] profit_trades_process_row_stopped_with_position bot=%s trade_block_id=%s status=%s",
            bot_name,
            trade_block_id,
...
def _collect_active_bot_process_rows(
    ...
    for entry in entries:
        ...
        row, warning = _collect_process_rows_for_bot(
            profile, bot_name, paths, confirmed_index=confirmed_index
        )
        if warning:
    ...
    if rows:
        ...
        logger.info(
            "[dashboard] profit_trades_active_process_records %s",
            json.dumps(payload, default=str),
        )
...
async def api_profit_trade_details(
    ...
    match["end_profit"] = end_profit
    match["profit_usdt"] = end_profit
    match["total_trade_pnl"] = end_profit
    payload = {
        "profile": profile,
        "bot_name": bot_name,
        "bot_side": bot_side,
...
def _build_profit_trade_filtered_rows(
    ...
    *,
    ...
    confirmed_rows_flat = _flatten_confirmed_pnl_index(confirmed_index)
    base_trades, warnings, _base_pagination = _load_trade_blocks_for_profile(
        ...
        paginate=False,
        bot_side=bot_side,
        confirmed_rows=confirmed_rows_flat,
    )
    ...
    process_rows, process_warnings = _collect_active_bot_process_rows(
        profile, bot_side, confirmed_index=confirmed_index
    )
    ...
    confirmed_start_times_all = _collect_confirmed_trade_start_times(
        profile, normalized_side, confirmed_rows=confirmed_rows_flat
    )
    ...
    for row in filtered_process_rows:
        ...
        confirmed_rows = confirmed_rows_all
        seen: set[str] = set()
        filled: list[dict[str, Any]] = []
        for entry in confirmed_rows:
            dedupe_key = _confirmed_row_dedupe_key(entry)
    ...
    confirmed_start_times = _collect_confirmed_trade_start_times(
        profile, bot_side, confirmed_rows=confirmed_rows_flat
    )
    summary_rows = [
    ...
    # Final-Exit-Status pro trade_block_id global erfassen (unabhängig von bot_side und Row-Typ).
    final_exit_confirmed_ids: set[str] = set()
    final_exit_meta: dict[str, dict[str, Any]] = {}
    candidate_tbids: set[str] = set()
    ...
    # anzeigen. Falls keine Summary-Zeile existiert, Process-Zeilen in geschlossene
    # History-Zeilen ummappen.
    if final_exit_confirmed_ids:
        summary_tbid_set: set[str] = {
            str(row.get("trade_block_id") or "").strip()
            for row in summary_rows
            if row.get("trade_block_id")
        }
        process_rows_before = len(filtered_process_rows)
        new_process_rows: list[dict[str, Any]] = []
        removed_process_rows: list[dict[str, Any]] = []
        for row in filtered_process_rows:
            tbid = str(row.get("trade_block_id") or "").strip()
            if not tbid or tbid not in final_exit_confirmed_ids:
                new_process_rows.append(row)
                continue
            meta = final_exit_meta.get(tbid) or {}
            if tbid in summary_tbid_set:
                removed_process_rows.append(
                    {
                        "bot_name": row.get("bot_name"),
                        "symbol": row.get("symbol"),
                        "trade_block_id": tbid,
            ...
            new_process_rows.append(converted)
            log_payload = {
                "profile": profile,
                "bot_side": normalized_side,
                "trade_block_id": tbid,
                ...
                "summary_rows_count": len(summary_rows),
                "original_status": row.get("status"),
                "selected_record_status": converted.get("status"),
                "selected_record_is_process": bool(converted.get("is_process")),
                "active_order_count": len(converted.get("active_orders") or []),
                "source": "process_to_history",
            }
            logger.info(
                "[dashboard] dashboard_profit_row_status_forced_closed %s",
                json.dumps(log_payload, default=str),
        ...
        filtered_process_rows = new_process_rows
        process_rows_after = len(filtered_process_rows)
        if removed_process_rows:
            payload = {
                "profile": profile,
                "bot_side": normalized_side,
                "summary_rows_count": len(summary_rows),
                "process_rows_count_before": process_rows_before,
                "process_rows_count_after": process_rows_after,
                "removed_process_rows": removed_process_rows,
            }
            logger.info(
                "[dashboard] dashboard_process_row_suppressed_by_final_exit %s",
                json.dumps(payload, default=str),
            )
