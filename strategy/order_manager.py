...
logger = logging.getLogger("psrh.order_manager")
...
class BybitOrderManager:
    ...
    def _post(self, path: str, body: str, timeout: int = 10) -> Mapping[str, Any] | None:
        ...
        try:
            ...
            if data.get("retCode") != 0:
                logger.warning("Bybit POST %s returned %s %s", path, data.get("retCode"), data.get("retMsg"))
                return None
            return data
    ...
    def _get(
        self, path: str, params: Mapping[str, Any], timeout: int = 10
        ...
        try:
            ...
            data = resp.json()
            if data.get("retCode") != 0:
                logger.warning("Bybit GET %s returned %s %s", path, data.get("retCode"), data.get("retMsg"))
                return None
            return data
        except Exception as exc:
    ...
    def place_market_order(
        ...
        *,
        ...
        if normalized_qty <= 0:
            logger.warning(
                "Bybit market order skipped due to zero qty for %s",
                symbol.upper(),
            )
            return None
        body = {
    ...
    def place_reduce_market_order(
        ...
        *,
        ...
        if normalized_qty <= 0:
            logger.warning(
                "Bybit reduce market order skipped due to zero qty for %s",
                symbol.upper(),
            )
            return None
        body = {
    ...
    def set_take_profit(self, symbol: str, tp_price: float, position_idx: int = 1) -> Mapping[str, Any] | None:
        return self.set_position_trading_stop(
            symbol=symbol,
    ...
    def normalize_qty(
        ...
        if qty_step > 0:
            normalized = (requested / qty_step).to_integral_value(
                rounding=ROUND_DOWN
            ) * qty_step
        ...
        if max_order_qty > 0 and normalized > max_order_qty:
            ...
            if qty_step > 0:
                normalized = (normalized / qty_step).to_integral_value(
                    rounding=ROUND_DOWN
                ) * qty_step
