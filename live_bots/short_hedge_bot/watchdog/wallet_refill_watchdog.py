...
CONFIG_PATH = PROJECT_ROOT / "live_bots" / "100_50_hedge_bot" / "config" / "config.yaml"
...
def resolve_bot_metadata(bot_name: str, logger: logging.Logger) -> tuple[str, str] | None:
    ...
    Wichtig für short_hedge_bot:
    - Runtime/Reserved/State enthalten den echten aktuell reservierten Coin.
    - config/fixed_cycle_config.json kann stale sein, z.B. ENAUSDT.
    """
    bot_name = bot_name.lower()
    ...
    # 1) Runtime config: highest priority, normally contains current reserved/active symbol.
    ...
    # 2) Reserved coin file: active scanner/bot reservation.
    ...
    # 3) Current state: fallback when runtime file is missing.
    ...
    # 4) Static config only as final fallback. This can be stale.
