...
source "${BOT_GROUP_DIR}/shared_scripts/load_bybit_env.sh" "${BOT_NAME}" "short"
...
if [[ -z "${BYBIT_API_KEY:-}" || -z "${BYBIT_API_SECRET:-}" ]]; then
  echo "ERROR: API credentials not set for ${BOT_NAME}" >&2
  exit 1
fi
...
def close(side_label, qty):
    ...
    if not resp:
        print(f"Failed to submit reduce-only market order for {side_label}", file=sys.stderr)
        sys.exit(1)
