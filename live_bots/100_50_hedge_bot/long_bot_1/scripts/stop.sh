...
if [[ ! -f "${PID_FILE}" ]]; then
  echo "long_bot_1 PID file not found; nothing to stop"
  exit 0
fi
...
if [[ -z "${PID}" || ! "${PID}" =~ ^[0-9]+$ ]]; then
  rm -f "${PID_FILE}"
  echo "long_bot_1 PID file empty or invalid, removed"
  exit 0
fi

if ! kill -0 "${PID}" 2>/dev/null; then
  rm -f "${PID_FILE}"
  echo "no long_bot_1 process with PID=${PID}"
  exit 0
fi
...
echo "Stopping long_bot_1 (PID=${PID})..."
...
if kill -0 "${PID}" 2>/dev/null; then
  echo "long_bot_1 still running, sending SIGKILL"
...
if kill -0 "${PID}" 2>/dev/null; then
  ...
  if [[ "${PROC_STATE}" == Z* ]]; then
    echo "long_bot_1 PID=${PID} is zombie after stop; removing PID file"
  else
    echo "long_bot_1 PID=${PID} still alive after SIGKILL (state=${PROC_STATE})" >&2
    exit 1
  fi
fi
rm -f "${PID_FILE}"
