...
source "${PROJECT_ROOT}/scripts/load_bybit_env.sh" long_bot_1 long

echo "[long_bot_1] Running hard reset..."
set +e
"${HARD_RESET_SCRIPT}"
RESET_CODE=$?
set -e

echo "[long_bot_1] hard-reset exit code: ${RESET_CODE}"

if [[ "${RESET_CODE}" -ne 0 ]]; then
  echo "[ERROR] long_bot_1 hard-reset failed (exit_code=${RESET_CODE}); aborting start." >&2
  exit 1
fi

echo "[long_bot_1] Cleaning log files..."
"${CLEAN_LOGS_SCRIPT}"
...
# ensure env refreshed before startup
source "${PROJECT_ROOT}/scripts/load_bybit_env.sh" long_bot_1 long
...
if ! kill -0 "${PID}" 2>/dev/null; then
  echo "[ERROR] long_bot_1 failed to stay running after start" >&2
  echo "[ERROR] Last stdout log lines:" >&2
  tail -n 80 "${LOG_DIR}/fixed_cycle_runner.stdout.log" >&2 || true
  rm -f "${PID_FILE}"
  exit 1
fi
echo "Fixed-cycle long_bot_1 started via nohup (${LOG_DIR}/fixed_cycle_runner.stdout.log)"
