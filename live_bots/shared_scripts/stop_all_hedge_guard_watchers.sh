...
SHORT_SAFETY_SCRIPT="${PROJECT_ROOT}/live_bots/short_hedge_bot/shared_scripts/stop_safety_order_watchdog.sh"
...
if [[ -x "${SHORT_SAFETY_SCRIPT}" ]]; then
  echo "[INFO] Stopping Short safety watchdog for ${SHORT_PROFILE}..."
  if ! bash "${SHORT_SAFETY_SCRIPT}" "${SHORT_PROFILE}"; then
    echo "[stop_all] short safety script failed" >&2
    errors=1
  fi
else
  echo "[stop_all] missing: ${SHORT_SAFETY_SCRIPT}" >&2
  errors=1
fi
