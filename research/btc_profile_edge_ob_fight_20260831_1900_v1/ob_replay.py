"""Self-contained OB200 v3 hour-archive replay for this case study.

Uses chunked zstd reads (stream_reader.readline is broken on these archives).
Does not modify dashboard/research_charts/ob200_walls.py.
"""
...
def find_hour_segment(symbol: str, at: datetime) -> Path:
    ...
    path = SHADOW / symbol / f"{hour.year:04d}" / f"{hour.month:02d}" / f"{hour.day:02d}" / name
    if not path.is_file():
        hits = list((SHADOW / symbol).rglob(name))
        if not hits:
...
def replay_as_of(symbol: str, at: datetime) -> dict[str, Any]:
    at = at.astimezone(timezone.utc) if at.tzinfo else at.replace(tzinfo=timezone.utc)
    path = find_hour_segment(symbol, at)
...
def replay_hour_at_cutoffs(
    symbol: str, hour_start: datetime, cutoffs: list[datetime]
) -> list[dict[str, Any]]:
    """Single pass over one hour archive; emit book snapshots for each cutoff."""
    from datetime import timedelta
    ...
    path = find_hour_segment(symbol, hour_start)
    ordered = sorted(
        {
...
def extract_walls(book_snap: dict[str, Any], *, max_walls: int = 10) -> list[dict[str, Any]]:
    ...
    def side_walls(levels: list[tuple[Decimal, Decimal]], side: str) -> list[dict[str, Any]]:
        if not levels or mid <= ZERO:
            return []
        thr = mid * WALL_MAX_BPS / Decimal("10000")
