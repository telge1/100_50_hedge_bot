...
class RecoveryBotStateTests(unittest.TestCase):
    ...
    def test_tracker_fields_initially_empty(self) -> None:
        ...
        self.assertIsNone(tracker.final_exit_candle_index)
        self.assertEqual(tracker.final_exit_combined_pnl, 0.0)
        self.assertEqual(tracker.final_exit_long_qty, 0.0)
        self.assertEqual(tracker.final_exit_short_qty, 0.0)
        self.assertIsNone(tracker.waiting_for_reload_since_candle_index)
        self.assertEqual(tracker.reload_count, 0)
        self.assertFalse(tracker.reload_attempted)
        self.assertIsNone(tracker.reload_candle_index)
        self.assertEqual(tracker.reload_long_qty, 0.0)
        self.assertEqual(tracker.reload_short_qty, 0.0)
        self.assertIsNone(tracker.reload_reason)
