"""CLI: Orderflow absorption pattern audit (read-only)."""
...
from research.regime_scanner.orderflow_absorption.audit import run_audit
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="run_orderflow_absorption_pattern_audit")
    p.add_argument("--symbols", required=True)
    p.add_argument("--start", required=True)
    p.add_argument("--end", required=True)
    p.add_argument("--lookbacks", default="6,12,24")
    p.add_argument("--horizons", default="3,6,12")
    p.add_argument("--move-thresholds", default="0.0025,0.005,0.01")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for candidate in (
        Path("research/regime_scanner/orderflow_absorption/feature_semantics.md"),
        Path(__file__).resolve().parent / "orderflow_absorption" / "feature_semantics.md",
    ):
    ...
    print(
        ...
        f"patterns_f1={result.get('pattern_counts_f1')} decision={result.get('decision')} "
