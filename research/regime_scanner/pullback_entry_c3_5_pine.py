...
def build_pullback_entry_pine(*, title: str | None = None) -> str:
    ...
    lines = [
        *build_pine_header(title or "C3.5 Pullback Entry Diagnose"),
        ...
        'showSetupSpan = input.bool(false, "Show setup state span (bg)")',
        'showStateBackground = input.bool(false, "Alias: show setup state bgcolor")',
        'maxReadyAgeInput = input.int(0, "Max ready age (0=off / R0)", minval=0, maxval=50)',
        'oppositeVetoInput = input.string("none", "Opposite veto mode", options=["none", "trigger_bar", "since_ready", "lookback_1", "lookback_2", "lookback_3"])',
