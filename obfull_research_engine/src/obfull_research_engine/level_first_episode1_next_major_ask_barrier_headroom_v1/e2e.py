"""E2E A/B for next-major-ask-barrier headroom."""
...
from . import ALLOW_CLICKHOUSE_WRITES, VERDICT_PROVEN
from .pipeline import run_headroom

RESULTS_ROOT = ENGINE_ROOT / "results" / "level_first_episode1_next_major_ask_barrier_headroom_v1"
...
def run_e2e(*, run_key: str, out_dir: Path | None = None) -> dict[str, Any]:
    ...
    result = run_headroom(run_key=run_key, out_dir=out_dir)
    pids_after = _pids()
    ...
    pb = out_dir / "pflichtbericht.json"
    if pb.exists():
        report = json.loads(pb.read_text(encoding="utf-8"))
        report["run_key"] = run_key
        report["live_safety"] = {
            "pids_unchanged": manifest["pids_unchanged"],
            "frozen_hashes_unchanged": manifest["frozen_hashes_unchanged"],
            "clickhouse_writes": False,
            "commit": False,
            "push": False,
        }
        atomic_write_json(pb, report)
    return manifest
...
def compare_ab(dir_a: Path, dir_b: Path) -> dict[str, Any]:
    ...
    return {
        "ok": (
            ...
            and a.get("verdict") == VERDICT_PROVEN
            and b.get("verdict") == VERDICT_PROVEN
        ),
            ...
            json.dumps(
                {
                    ...
                    "explicit_answers": man.get("explicit_answers"),
                },
                indent=2,
                default=str,
            )
