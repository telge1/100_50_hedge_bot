"""E2E A/B for wall migration structure."""
...
from . import ALLOW_CLICKHOUSE_WRITES, RUN_PREFIX, VERDICT_STRUCTURE_PROVEN, VERDICT_NOT_OBSERVED
from .pipeline import run_wall_migration_structure

RESULTS_ROOT = ENGINE_ROOT / "results" / "level_first_episode1_wall_migration_structure_v1"
...
def run_e2e(*, run_key: str, out_dir: Path | None = None) -> dict[str, Any]:
    ...
    result = run_wall_migration_structure(run_key=run_key, out_dir=out_dir)
    pids_after = _pids()
            ...
            json.dumps(
                {
                    ...
                    "n_gens_ep4": man.get("n_gens_ep4"),
                    "n_pre_existing_above": man.get("n_pre_existing_above"),
                    "n_post_breach_new": man.get("n_post_breach_new"),
                    "ask_centroid_moved_higher": man.get("ask_centroid_moved_higher"),
                },
                indent=2,
            )
        )
