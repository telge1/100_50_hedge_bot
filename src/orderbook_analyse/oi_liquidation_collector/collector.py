...
from .health_snapshot import HealthSnapshot, evaluate_health, write_health_atomic
...
from .spool import DurableSpool, SpoolCorruptError, SpoolFullError, SpoolMetaError
...
class WriterDeadError(RuntimeError):
    """Writer task died or persistence lag exceeded — process must exit nonzero."""
...
def default_client_factory(settings: OICollectorSettings):
    def factory():
        ...
        # Inserts do not need server-side sessions. Autogenerated session_id is what
        # produced SESSION_IS_LOCKED after transport faults (see resilience audit).
...
class CollectorStats:
    ...
    last_oi_received_at: datetime | None = None
...
class OILiquidationCollector:
    ...
    def check_liveness(self, now_mono: float | None = None) -> None:
        ...
        if not self.writer.is_alive():
            reason = f"writer_dead:{self.writer.fatal}"
            self.stats.fail_fast_reason = reason
            raise WriterDeadError(reason)
        if pong_timed_out(
        ...
        # Receiving market data but not persisting → fail-fast (spool may still hold data).
        if self.stats.last_oi_received_at is not None:
            now_unix = time.time()
            oi_age = now_unix - self.stats.last_oi_received_at.timestamp()
            recently_receiving = oi_age < self.settings.oi_stale_fail_sec
            last_ins = self.writer.last_successful_insert_unix
            if recently_receiving:
                if last_ins is None:
                    # Allow startup grace equal to persistence_lag_fail_sec from first OI.
                    if oi_age >= self.settings.persistence_lag_fail_sec:
                        reason = "persistence_lag:no_successful_insert"
                        self.stats.fail_fast_reason = reason
                        raise WriterDeadError(reason)
                else:
                    insert_age = now_unix - last_ins
                    if insert_age >= self.settings.persistence_lag_fail_sec:
                        reason = f"persistence_lag:{insert_age:.1f}s"
                        self.stats.fail_fast_reason = reason
                        raise WriterDeadError(reason)

    def build_health_snapshot(self) -> HealthSnapshot:
        now = time.time()
        spool_n, spool_b = (0, 0)
        if self.writer.spool is not None:
            try:
                spool_n, spool_b = self.writer.spool.unacked_stats()
            except Exception:  # noqa: BLE001
                spool_n, spool_b = -1, -1
        snap = HealthSnapshot(
            process_alive=True,
            websocket_alive=bool(self.stats.ws_connected and self.stats.ping_ok),
            writer_alive=self.writer.is_alive(),
            clickhouse_reachable=bool(self.writer.clickhouse_reachable),
            last_ws_message_ts=self.stats.last_received_at.timestamp() if self.stats.last_received_at else None,
            last_oi_received_ts=self.stats.last_oi_received_at.timestamp() if self.stats.last_oi_received_at else None,
            last_liquidation_received_ts=(
                self.stats.last_liquidation_time.timestamp() if self.stats.last_liquidation_time else None
            ),
            last_successful_insert_ts=self.writer.last_successful_insert_unix,
            last_oi_persisted_ts=self.writer.last_oi_persisted_unix,
            last_liquidation_persisted_ts=self.writer.last_liquidation_persisted_unix,
            queue_depth=self.writer.queue_size,
            queue_capacity=self.writer.queue_maxsize,
            queue_drop_count=self.writer.queue_drops,
            writer_error_count=self.writer.insert_errors,
            writer_restart_count=self.writer.writer_restart_count,
            clickhouse_reconnect_count=self.writer.clickhouse_reconnect_count,
            websocket_reconnect_count=self.stats.reconnect_count,
            spool_unacked_records=spool_n,
            spool_unacked_bytes=spool_b,
            pid=os.getpid(),
            collector_instance_id=self.instance_id,
        )
        if snap.last_oi_received_ts is not None and snap.last_successful_insert_ts is not None:
            snap.persistence_lag_seconds = max(0.0, snap.last_oi_received_ts - snap.last_successful_insert_ts)
        return evaluate_health(
            snap,
            persistence_lag_fail_sec=self.settings.persistence_lag_fail_sec,
            oi_stale_fail_sec=self.settings.oi_stale_fail_sec,
            now=now,
        )

    def write_local_health(self) -> HealthSnapshot:
        snap = self.build_health_snapshot()
        write_health_atomic(self.settings.health_file, snap)
        return snap

    async def _health_file_loop(self) -> None:
        while not self._stop.is_set():
            try:
                snap = self.write_local_health()
                if snap.health_status == "RED" and (
                    not self.writer.is_alive() or "persistence_lag" in snap.health_reasons or "writer_dead" in snap.health_reasons
                ):
                    self.stats.fail_fast_reason = ",".join(snap.health_reasons) or "health_red"
                    logger.critical("health RED fail-fast: %s", self.stats.fail_fast_reason)
                    self.request_stop()
                    return
            except Exception:  # noqa: BLE001
                logger.exception("health file write failed")
            try:
    ...
    async def _on_ticker(self, msg: dict[str, Any], received: datetime) -> None:
        ...
        if result.get("changed"):
            ...
            if row:
                self.stats.last_oi_received_at = received
    ...
    async def _emit_5s(self) -> None:
        ...
        for symbol, st in self.states.items():
            ...
            self.stats.last_oi_received_at = now
        if rows:
    ...
    async def run(self) -> dict[str, Any]:
        ...
        health_task = asyncio.create_task(self._health_file_loop())
        ...
        try:
            while not self._stop.is_set():
                try:
                    if not self.writer.is_alive():
                        raise WriterDeadError(f"writer_dead:{self.writer.fatal}")
                    if self.stats.fail_fast_reason:
                        raise WriterDeadError(self.stats.fail_fast_reason)
                    await self._session(deadline)
                    break
                except WriterDeadError as exc:
                    fatal = exc
                    logger.critical("fail-fast: %s", exc)
                    self.request_stop()
                    break
                except SpoolFullError as exc:
                    fatal = WriterDeadError(f"spool_full:{exc}")
                    self.stats.fail_fast_reason = str(fatal)
                    logger.critical("fail-fast: %s", fatal)
                    self.request_stop()
                    break
                except SpoolMetaError as exc:
                    fatal = WriterDeadError(f"spool_meta:{exc}")
                    self.stats.fail_fast_reason = str(fatal)
                    logger.critical("fail-fast: %s", fatal)
                    self.request_stop()
                    break
                except SpoolCorruptError as exc:
                    fatal = WriterDeadError(f"spool_corrupt:{exc}")
                    self.stats.fail_fast_reason = str(fatal)
                    logger.critical("fail-fast: %s", fatal)
                    self.request_stop()
                    break
                    ...
                    if self._stop.is_set():
                        if self.stats.fail_fast_reason:
                            fatal = WriterDeadError(self.stats.fail_fast_reason)
                        break
            ...
            try:
                ...
                logger.exception("final health enqueue failed")
            try:
                self.write_local_health()
            except Exception:  # noqa: BLE001
                logger.exception("final health file failed")
        result = {
            ...
            "fail_fast_reason": self.stats.fail_fast_reason or (str(fatal) if fatal else None),
        }
        if fatal is not None:
            raise WriterDeadError(str(fatal)) from fatal
        return result
    ...
    try:
        ...
        if not settings.disable_spool:
            try:
                ...
                logger.exception("spool corrupt — refuse start (fail-closed)")
                return 2
        writer = AllowlistedWriter(
        ...
        if spool is not None:
            unacked = list(spool.iter_unacked())
            if unacked:
                logger.warning("replaying %s unacked spool records", len(unacked))
                await writer.enqueue_spool_records(unacked)
        ...
        try:
            ...
            exit_code = 1
            logger.critical("collector abort: %s", exc)
            try:
                await writer.stop()
            except Exception:  # noqa: BLE001
                logger.exception("writer stop after fatal")
            print(orjson.dumps({"fatal": str(exc), "exit_code": 1}, default=str).decode())
        except SpoolFullError as exc:
            exit_code = 1
            logger.critical("spool full: %s", exc)
            try:
                await writer.stop()
            except Exception:  # noqa: BLE001
                logger.exception("writer stop after spool full")
        finally:
            if spool is not None:
                spool.close()
        return exit_code
