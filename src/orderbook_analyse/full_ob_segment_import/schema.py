"""ClickHouse DDL for research_full_ob_import_pilot_v1 (isolated)."""
...
CREATE TABLE IF NOT EXISTS {database}.full_ob_import_state
(
...
CREATE TABLE IF NOT EXISTS {database}.full_ob_events
(
    fight_event_id String,
    symbol LowCardinality(String),
...
CREATE TABLE IF NOT EXISTS {database}.full_ob_segments
(
    segment_id FixedString(64),
    fight_event_id String,
    symbol LowCardinality(String),
...
CREATE TABLE IF NOT EXISTS {database}.full_ob_records
(
...
CREATE TABLE IF NOT EXISTS {database}.full_ob_signals
(
    signal_id String,
...
CREATE TABLE IF NOT EXISTS {database}.signal_analysis_contracts
(
    contract_id String,
    signal_id String,
    parent_event_id String,
...
CREATE VIEW IF NOT EXISTS {database}.v_full_ob_records_canonical AS
SELECT *
FROM {database}.full_ob_records
...
CREATE VIEW IF NOT EXISTS {database}.v_full_ob_checkpoints AS
SELECT *
FROM {database}.v_full_ob_records_canonical
WHERE record_kind IN ('INITIAL_CHECKPOINT', 'RESYNC_CHECKPOINT');

CREATE VIEW IF NOT EXISTS {database}.v_full_ob_book_deltas AS
SELECT *
FROM {database}.v_full_ob_records_canonical
WHERE record_kind = 'BOOK_DELTA';

CREATE VIEW IF NOT EXISTS {database}.v_full_ob_markers AS
SELECT *
FROM {database}.v_full_ob_records_canonical
WHERE record_kind IN ('EVENT_MARKER', 'EVENT_END', 'RESYNC_BOUNDARY', 'NESTED_PROFILE_EDGE_SIGNAL');

CREATE VIEW IF NOT EXISTS {database}.v_full_ob_level_changes AS
SELECT
    record_id,
    fight_event_id,
    symbol,
...
FROM {database}.v_full_ob_book_deltas
...
CREATE VIEW IF NOT EXISTS {database}.v_full_ob_signals_canonical AS
SELECT *
FROM {database}.full_ob_signals
...
CREATE VIEW IF NOT EXISTS {database}.v_signal_analysis_contracts_canonical AS
SELECT *
FROM {database}.signal_analysis_contracts
...
CREATE VIEW IF NOT EXISTS {database}.v_full_ob_import_state_canonical AS
SELECT *
FROM {database}.full_ob_import_state
...
def render_schema(database: str) -> str:
    return SCHEMA_SQL.format(database=database)
